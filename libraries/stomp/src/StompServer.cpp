 ...
 **************************************************************************/
  ...
  void StompServer::onDescribeStats() {
    ...
    describe_stat("num.peers", "main/name peers", openstats::graphTypeGauge, openstats::dataTypeInt, openstats::useTypeMean);
...
//    } // while
  ...
  bool StompServer::_process_peer(StompPeer *peer) {
    ...
    for(size_t i=0; i < _max_work && peer->process(); i++);
  ...
  void StompServer::_process(StompPeer *peer, StompFrame *frame) {
    ...
    if (connect_only) {
      peer->send_error("must send CONNECT first");
      return;
    } // if
  ...
  void StompServer::_process_connect(StompPeer *peer, StompFrame *frame) {
    if (!frame->is_header("login")) {
      peer->send_error("missing login");
      return;
    } // if

    if (!frame->is_header("passcode")) {
      peer->send_error("missing passcode");
      return;
    } // if

    string login = frame->get_header("login");
    string passcode = frame->get_header("passcode");

    if (!login.length()) {
      peer->send_error("must specify login");
      return;
    } // if
  ...
  void StompServer::_process_ack(StompPeer *peer, StompFrame *frame) {
    ...
    string message_id = frame->get_header("message-id");
    string subscription = frame->get_header("subscription");

    peer->ack(subscription, message_id);
...
//smesg->dont_delete();
      ...
      StompMessage *smesg = new StompMessage( st.trail(0), frame->body(), 3600);
      smesg->copy_headers_from(frame);	// copy headers
//smesg->dont_delete();
      exch->post(smesg);	// post retains
      smesg->release();
...
//      enqueue_queue(unsent);
  ...
  void StompServer::onConnect(const Connection *con) {
    ...
    try {
      ...
      peer->elogger(elogger(), elog_name() );
...
//      out << "[ STOMP Packet from " << lis->peer_str << " " << std::setprecision(3) << std::fixed << peer->pps() << " pps ]";
      LOG(LogDebug, << "<" << StringTool::ppad(out.str(), "-", 79) << std::endl);
      LOG(LogDebug, << StringTool::hexdump(lis->in, "<   ") << "<" << std::endl);
    } // if
  ...
  bool StompServer::onPeerWake(const Peer *lis) {
    openframe::scoped_lock slock(&_peers_l);
    peers_itr itr = _peers.find(lis->sock);
