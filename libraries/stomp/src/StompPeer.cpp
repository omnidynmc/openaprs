 ...
 **************************************************************************/
  ...
  const bool StompPeer::checkLogstats() {
    ...
    for(subscriptions_citr citr=_subscriptions.begin(); citr != _subscriptions.end(); citr++) {
      subscriptions_citr ncitr = citr;
      ncitr++;
      char c = (ncitr == _subscriptions.end() ? '`' : '|');
  ...
  void StompPeer::onError(StompFrame *frame) {
