...
class Parser : public stomp::StompParser {
  public:
    ...
    virtual void onError(stomp::StompFrame *frame) {
  ...
  for(size_t i=0; i < 100; i++) {
    ...
    for(size_t i=0; i < 100; i++) {
      ...
      s << "SEND\n"
        ...
        << "\n"
        << openframe::StringTool::randstr(10, 10) << "\n"
        << '\0' << "\n";
      out += s.str();
    } // for
...
//std::cout << "SENT(" << openframe::StringTool::safe(out.substr(0, len)) << ")" << std::endl;
//std::cout << openframe::StringTool::safe(out.substr(0, len)) << std::endl;
