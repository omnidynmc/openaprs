 ...
 **************************************************************************/
  ...
  const int stdinVERIFY::Execute(COMMAND_ARGUMENTS) {
    ...
    if (aprs::StringTool::ereg("[@]", find, rl))
      numRows = mgmt->dbi()->verifyUserByEmail(find);
    else if (aprs::StringTool::ereg("^[0-9]+$", find, rl))
      numRows = mgmt->dbi()->verifyUserById(find);
    else
      numRows = mgmt->dbi()->verifyUserByCallsign(find);

    //cout << numRows << " row" << (numRows != 1 ? "s" : "") << " affected." << endl;
