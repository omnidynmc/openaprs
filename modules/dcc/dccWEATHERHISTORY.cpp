 ...
 **************************************************************************
 $Id: dccWEATHERHISTORY.cpp,v 1.1 2005/11/21 18:16:04 omni Exp $
 **************************************************************************/
...
namespace dcc {
   ...
   * LM:		Limit
   *
   * SYNTAX: .PH CL:<callsign(wildcards[*,?])>[|AG:age|ST:start|LM:limit]
   *
   */
  const int dccWEATHERHISTORY::Execute(COMMAND_ARGUMENTS) {
    DBI *dbi;
    DBI::setMapType sm;
    ...
    numReturned = dbi->weather->HistoryByCallsign(callsign, start_ts, end_ts, limit, sm);
    ...
    app->writeLog(MODULE_DCC_LOG_NORMAL, "*** Module DCC: (WH) %s received %d of %d matches (%s seconds)",
                      dccuser->callsign().c_str(),
                      sm.size(),
                      numReturned,
                      y);

    dccuser->datapoint("num.op.weather.history");
    return CMDERR_SUCCESS;
  } // dccWEATHERHISTORY::Execute
