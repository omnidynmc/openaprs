 ...
 **************************************************************************
 $Id: dccADDMODULE.cpp,v 1.1 2005/11/21 18:16:04 omni Exp $
 **************************************************************************/
...
using namespace openaprs;
  ...
  const int dccADDMODULE::Execute(COMMAND_ARGUMENTS) {
    ...
    app->Module(name, ModuleAction::add);
    dccuser->reply("replyOk");

/*
    if (app->module->add(name, true)) {
      dcc->replyToUser(dccuser, "replyOk");
      app->writeLog(OPENAPRS_LOG_NORMAL, "*** Module: %s loaded.",
                        name.c_str());
    } // if
    else
      dcc->replyToUser(dccuser, "errModuleNotFound");
*/

    return CMDERR_SUCCESS;
  } // dccADDMODULE::Execute
} // namespace dcc
