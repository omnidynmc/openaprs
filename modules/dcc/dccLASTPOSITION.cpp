 ...
 **************************************************************************
 $Id: dccLASTPOSITION.cpp,v 1.1 2005/11/21 18:16:04 omni Exp $
 **************************************************************************/
...
namespace dcc {
   ...
   *
   */
  const int dccLASTPOSITION::Execute(COMMAND_ARGUMENTS) {
    DBI *dbi;
    DBI::setMapType sm;
    ...
    numReturned = dbi->position->LastByCallsign(callsign, start_ts, end_ts, limit, sm);
    ...
    dccuser->sendf("305 RS:%d|MH:%d|SW:%s|MS:%d of %d matches returned (%s seconds).\n", 
                sm.size(),
                numReturned,
                y, 
                sm.size(),
    ...
    app->writeLog(MODULE_DCC_LOG_NORMAL, "*** Module DCC: (LP) %s received %d of %d matches (%s seconds)",
                      dccuser->callsign().c_str(),
                      sm.size(),
                      numReturned,
                      y);

    dccuser->datapoint("num.op.last.position");
    return CMDERR_SUCCESS;
  } // dccLASTPOSITION::Execute

  /**************************************************************************
   ** dccLASTPOSITIONBBOX Class                                            **
   **************************************************************************/
   ...
   * AG:		Age in seconds
   *
   * SYNTAX: .LP BB:<lat1>,<lon1>,<lat2>,<lon2>[|AG:age|ST:start]
   *
   */
  const int dccLASTPOSITIONBBOX::Execute(COMMAND_ARGUMENTS) {
    DBI *dbi;
    DBI::setMapType sm;
    DBI::resultMapType rm;
    DBI::resultMapType::iterator ptr;
    ...
    numReturned = dbi->position->LastByLatLong(lat1,
                                                       lon1,
                                                       lat2,
                                                       lon2,
                                                       start_ts, 
    ...
    app->writeLog(MODULE_DCC_LOG_NORMAL, "*** Module DCC: (LB) %s received %d of %d matches (%s seconds)",
                      dccuser->callsign().c_str(),
                      sm.size(),
                      numReturned,
                      y);
    ...
    dccuser->datapoint("num.op.last.position");
    return CMDERR_SUCCESS;
  } // dccLASTPOSITIONBBOX::Execute

  /**************************************************************************
   ** dccLASTPOSITIONCENTER Class                                          **
   **************************************************************************/
   ...
   * AG:		Age in seconds
   *
   * SYNTAX: .LC CN:<lat>,<lon>|RG:<range in meters>[|AG:age|ST:start]
   *
   */
  const int dccLASTPOSITIONCENTER::Execute(COMMAND_ARGUMENTS) {
    DBI *dbi;
    DBI::setMapType sm;
    DBI::resultMapType rm;
    DBI::resultMapType::iterator ptr;
...
//cout << lat2 << " " << lon2 << endl;
    ...
    dccuser->sendf("305 RS:%d|MH:%d|SW:%s|BB:%f,%f,%f,%f|MS:%d of %d matches returned (%s seconds).\n", 
                sm.size(),
                numReturned,
                y, 
                lat1,
    ...
    app->writeLog(MODULE_DCC_LOG_NORMAL, "*** Module DCC: (LC) %s received %d of %d matches (%s seconds)",
                      dccuser->callsign().c_str(),
                      sm.size(),
                      numReturned,
                      y);
