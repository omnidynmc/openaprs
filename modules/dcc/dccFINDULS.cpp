...
namespace dcc {
  ...
  const int dccFINDULS::Execute(COMMAND_ARGUMENTS) {
    DBI *dbi;
    DBI::setMapType sm;
    ...
    if (ereg("^([*a-zA-Z0-9]{2,7})$", callsign, rl) < 1) {
    ...
    numReturned = dbi->uls->FindByCallsign(callsign, sm);
    ...
    dccuser->sendf("325 RS:%d|MH:%d|SW:%s|MS:%d of %d matches returned (%s seconds).\n", 
                sm.size(),
                numReturned,
                y,
                sm.size(),
    ...
    app->writeLog(MODULE_DCC_LOG_NORMAL, "*** Module DCC: (FL) %s received %d of %d matches (%s seconds)",
                      dccuser->callsign().c_str(),
                      sm.size(),
                      numReturned,
                      y);

    dccuser->datapoint("num.op.find.uls");
    return CMDERR_SUCCESS;
  } // dccFINDULS::Execute
} // namespace dcc
