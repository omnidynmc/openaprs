 ...
 **************************************************************************
 $Id: dccFREQUENCY.cpp,v 1.1 2005/11/21 18:16:04 omni Exp $
 **************************************************************************/
...
namespace dcc {
   ...
   * ST:		Start Timestamp
   *
   * SYNTAX: .FQ CL:<callsign(wildcards[*,?])>[|AG:age|ST:start|LM:limit]
   *
   */
  const int dccFREQUENCY::Execute(COMMAND_ARGUMENTS) {
    DBI *dbi;
    DBI::setMapType sm;
    ...
    numReturned = dbi->extra->FrequencyByCallsign(callsign, start_ts, end_ts, limit, sm);
    ...
    for(i=0; i < sm.size(); i++) {
      ...
      for(ptr = rm.begin(); ptr != rm.end(); ptr++) {
        if (!dccuser->findFieldByName(ptr->first, field))
          continue;
    ...
    dccuser->sendf("307 RS:%d|MH:%d|SW:%s|MS:%d of %d matches returned (%s seconds).\n", 
                sm.size(),
                numReturned,
                y, 
                sm.size(),
    ...
    app->writeLog(MODULE_DCC_LOG_NORMAL, "*** Module DCC: (FQ) %s received %d of %d matches (%s seconds)",
